...
float ModConfig::SideLightsVisibleRange = 80.0f;
...
void ModConfig::SaveSettings()
{
    ...
    generalSection->AddLine("; Range in degrees for the lights (when direction is set to SIDES) is visible. Default is 80.0");
    generalSection->AddFloat("side_lights_visible_range", ModConfig::SideLightsVisibleRange);
    generalSection->AddLine("");
...
void ModConfig::LoadSettings()
{
    ...
    if (generalSections.size() > 0)
    {
        ...
        generalSection->GetFloat("side_lights_visible_range", &ModConfig::SideLightsVisibleRange);
