...
void WindowLightGroups::CreateEditLightGroup(Window* parent, LightGroup* lightGroup)
{
    ...
    window->AddCheckbox(18, &lightGroup->fadeEnabled);
    
    window->AddIntRange(21, &lightGroup->fadeSpeed, 50, 2000, 10);
    
    window->AddCheckbox(121, &lightGroup->smoothLedFade);

    window->AddIntRange(122, &lightGroup->smoothLedFadeSpeed, 50, 2000, 10);
...
void WindowLightGroups::CreateEditPoint(Window* parent, LightGroup* lightGroup, Point* point)
{
    ...
    //====================================================
    // Small White Corona - Cor individual
    //====================================================

    window->AddCheckbox(118, &point->useCustomWhiteCoronaColor);

    auto button_whiteCoronaColor = window->AddButton(119);
    button_whiteCoronaColor->AddColorIndicator(&point->customWhiteCoronaColor);
    button_whiteCoronaColor->onClick = [point, window]() {
        Menu::AddColorMenu(window, &point->customWhiteCoronaColor);
    };
    
    auto whiteCount = window->AddIntRange(123, &point->whiteCoronaCount, 1, 100, 1);
    whiteCount->holdToChange = false;
    
    whiteCount->onValueChange = [window, lightGroup, point]()
{
    while (point->whiteCoronaOffsets.size() < (size_t)point->whiteCoronaCount)
        point->whiteCoronaOffsets.push_back(CVector(0.0f, 0.0f, 0.0f));

    while (point->whiteCoronaOffsets.size() > (size_t)point->whiteCoronaCount)
        point->whiteCoronaOffsets.pop_back();

    if (WindowEditing::SelectedWhiteCorona < 1)
        WindowEditing::SelectedWhiteCorona = 1;

    if (WindowEditing::SelectedWhiteCorona > point->whiteCoronaCount)
        WindowEditing::SelectedWhiteCorona = point->whiteCoronaCount;
};
    
    auto whiteSelected = window->AddIntRange(
    124,
    &WindowEditing::SelectedWhiteCorona,
    1,
    100,
    1
);

whiteSelected->holdToChange = false;
...
while (point->whiteCoronaOffsets.size() > (size_t)point->whiteCoronaCount)
{
    ...
    auto button_whiteCoronaPosition = window->AddButton(125);

button_whiteCoronaPosition->onClick = [point, window]()
{
    while (point->whiteCoronaOffsets.size() < (size_t)point->whiteCoronaCount)
        point->whiteCoronaOffsets.push_back(CVector(0.0f, 0.0f, 0.0f));

    if (WindowEditing::SelectedWhiteCorona > point->whiteCoronaOffsets.size())
        return;

    float moveSpeed = WindowEditing::MoveLightsSpeed * 0.01f;

    auto posWindow = Menu::AddPositionWindow(
        window,
        &point->whiteCoronaOffsets[WindowEditing::SelectedWhiteCorona - 1],
        -1000.0f,
        1000.0f,
        moveSpeed,
        [](){},
